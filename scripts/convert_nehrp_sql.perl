...
if (!$ARGV[0])
{
	print "Usage: perl convert_nehrp_sql.perl INPUT_FILE EDITION (GRID_SPACING) (OUT_FILE)\n";
	print "	To force the script to calculate grid spacing from the input file, set GRID_SPACING to \"CALC\".";
	exit;
}
...
while ($edition ne "" && $edition ne "nehrp-2015" && $edition ne "asce_41-2013,1e" && $edition ne "asce_41-2013,2e" &&
       $edition ne "ibc-2012" && $edition ne "asce-2010" && $edition ne "nehrp-2009" && $edition ne "aashto-2009" &&
	   $edition ne "ibc-2009" && $edition ne "asce_41-2006" && $edition ne "asce-2005" && $edition ne "nehrp-2003")
	{ print "Invalid edition.\n:"; exit; }
if ($edition eq "") { print "Using edition nehrp-2015.\n"; $edition = "nehrp-2015"; }
my $dcv = "NULL";

if ($edition eq "asce_41-2013,1e")
{
	$edition = "asce_41-2013";
	$dcv = "(SELECT id FROM us_design.design_code_variant WHERE code = 'BSE-1E' LIMIT 1)";
}

if ($edition eq "asce_41-2013,2e")
{
	$edition = "asce_41-2013";
	$dcv = "(SELECT id FROM us_design.design_code_variant WHERE code = 'BSE-2E' LIMIT 1)";
}
...
($record_number, $lat, $lon, $num_vals, $v1, $v2) = unpack("i f f s f f", $record);
my $rec5_lon = sprintf("%.4f", $lon);

# Calculate grid spacing.
my $grid_spacing = sprintf("%.4f", $rec5_lon - $rec4_lon);
if ($ARGV[2]) { if (lc($ARGV[2]) ne "calc") { $grid_spacing = $ARGV[2]; } }
...
BEGIN
	...
	INSERT INTO
		us_design.dataset
		(data_group_id, edition_id, design_code_variant_id, region_id, fa_table_id, fv_table_id, fpga_table_id, grid_spacing)
		VALUES
		(
			datagroup_rec_num,
...
for ($record_read_num = 4; ($record_read_num - 1) * $record_length < $bfile_size; $record_read_num++)
{
	...
	{
		print "	Bad data, #$record_read_num: ($record_number, $lat_str, $lon_str, $v1_str, $v2_str)\n";
